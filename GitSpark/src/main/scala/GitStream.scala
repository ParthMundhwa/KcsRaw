...
object GitStream {
  ...
  def main(args:Array[String]):Unit={
    ...
    val df = spark.readStream.format("kafka").option("kafka.bootstrap.servers", "kcs-soyou-stnn:9092").option("startingOffsets","earliest").option("subscribe", "streamdata").load()
    ...
    whitespace.writeStream.outputMode("append").format("org.elasticsearch.spark.sql").option("checkpointLocation","path-to-checkpointing1").option("es.port","9200").option("es.nodes","172.16.3.11").start("monika_stream_test/Gpcb_type").awaitTermination()
